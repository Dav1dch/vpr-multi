...
import tqdm
from datasets.augmentation import MinimizeTransform, TrainRGBTransform
...
class SevenScenesDatasets(Dataset):
    ...
    def __init__(
        ...
        dataset_path: str,
        query_filename: str,
        image_transform=TrainRGBTransform(aug_mode=1),
    ):
    ...
    def __getitem__(self, ndx):
        ...
        imgs = torch.stack(
            [
                ...
                # ),
                # self.image_transform(
                #     self.load_img(self.queries[max(ndx - 1, 0)].rel_scan_filepath)
                # ),
                # self.image_transform(
                #     self.load_img(self.queries[max(ndx, 0)].rel_scan_filepath)
                # ),
            ]
        )
        result = {}
