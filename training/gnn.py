...
import pickle
import dgl
from gnn_loss import ERFA, Roadmap, sigmoid, SmoothAP
import os
import argparse
...
def main():
    ...
    iou = (1 / (torch.cdist(train_pose, train_pose) + 0.12)).type(dtype=torch.float32)
            ...
            with tqdm.tqdm(
                ...
                for (
                    ...
                    with torch.enable_grad():
                        ...
                        loss += losses[-1].item()
                        if cnt % 64 == 0 or cnt == len(database_sets):
