        ...
        x.append(
            torch.stack(
                [
                    ...
                    for i in range(0, 2)
                    # load_data_item(set[max(elem_ndx - 1, 0)]["query"], params),
                    # load_data_item(set[elem_ndx]["query"], params),
                ]
            )
        )
        if len(x) == 10:
