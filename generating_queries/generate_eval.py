...
def construct_query_and_database_sets(root_name):
    ...
    for i in range(len(database_sets)):
        for j in range(len(database_sets)):
            for key in range(len(database_sets[j].keys())):
                ind_p = database_tree.query_radius([database_pose[key].tolist()], r=0.4)
                ind_p = ind_p[0]
                ind_p = ind_p[ind_p // 1000 != key // 1000]

                database_sets[j][key][i] = ind_p.tolist()
