...
def sigmoid_t(tensor, temp=0.01):
    ...
    return (
        sigmoid(tensor_one / 0.001, temp)
        + (sigmoid(tensor_two / 0.001, temp) + 0.5)
        + (10e3 * (tensor_three - 0.05) + sigmoid(tensor_three / 0.001, temp) + 0.5)
    )
...
class Roadmap(torch.nn.Module):
    ...
    def forward(self, sim_all, pos_mask_, gt_iou):
        ...
        Dpos = D * pos_mask
        Dneg = D * (~pos_mask)
        Dpos = sigmoid(Dpos, 0.0001)
        Dneg = sigmoid_t(Dneg, 0.01)
        D_ = Dpos + Dneg
        D_ = D * (1 - torch.eye(len(sim_mask))).to("cuda")
        D_pos = D_ * pos_mask
