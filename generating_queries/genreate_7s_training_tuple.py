...
def gen_tuple(scene, root_dir):
    ...
    train_positive = train_tree.query_radius(train_translations, r=0.2)
    ...
    file_path = os.path.join(root_name + "_train" + "_dist.pickle")
    ...
    file_path = os.path.join(root_name + "_test" + "_dist.pickle")
    with open(file_path, "wb") as handle:
        pickle.dump(queries, handle, protocol=pickle.HIGHEST_PROTOCOL)
